...
#pragma warning restore 1591
        {
            if (IsMissing(value))
            {
                ...
                {
                    //Yes, there is
                    List<TStore> storeList = RowKeyToStoreList[rowKey];
...
#pragma warning restore 1591
        {
            get
            {
                foreach (KeyValuePair<string, List<TStore>> rowKeyAndColIndexToVal in RowKeyToStoreList)
                {
                    string rowKey = rowKeyAndColIndexToVal.Key;
                    List<TStore> structArray = rowKeyAndColIndexToVal.Value;
...
#pragma warning restore 1591
        {
            get
            {
                foreach (List<TStore> structArray in RowKeyToStoreList.Values)
                {
        ...
        public void Write(TextWriter textWriter, ParallelOptions parallelOptions)
        {
            ...
            ParallelQuery<string> valueStringQuery =
            ...
            foreach (string rowKeyAndValString in valueStringQuery)
            {
            ...
            bool zeroIsOK, string fileMessageOrNull, TStore storeMissingValue, CounterWithMessages counterWithMessages)
        {
            foreach (string fileName in FileUtils.GetFiles(filePattern, zeroIsOK))
            {
                ...
                using (TextReader textReader = FileUtils.OpenTextStripComments(fileName))
                {
                    ...
                    while (null != (line = textReader.ReadLine()))
                    {
                        KeyValuePair<string, List<TStore>> rowKeyAndStructList = CreateRowKeyAndStructList(line, filePattern, colSerialNumberCollection.Count, staticStringiToStoreListDelegate, counterWithMessages);
        ...
        {
            IEnumerable<string> rowKeyQuery =
                from fileName in FileUtils.GetFiles(denseStructFilePattern, /*zeroIsOK*/ false)
                from rowKey in RowKeysInFile(fileName)
                select rowKey;

            List<string> rowKeyList = rowKeyQuery.Distinct().ToList();
            return rowKeyList;
        }
        ...
        public void GetInstanceFromSparseInternal(IEnumerable<RowKeyColKeyValue<string, string, TValue>> tripleEnumerable)
        {
            ...
            foreach (RowKeyColKeyValue<string, string, TValue> triple in tripleEnumerable)
            {
            ...
            foreach (List<TStore> storeList in RowKeyToStoreList.Values)
            {
                storeList.TrimExcess();
            }
        ...
        {
            SelectRowsAndColsView<string, string, TValue> selectRowsAndColsView = inputMatrix as SelectRowsAndColsView<string, string, TValue>;
            ...
            {
                DenseStructMatrix<TStore, TValue> parentMatrix = (DenseStructMatrix<TStore, TValue>)selectRowsAndColsView.ParentMatrix;
