...
namespace Bio.Web.Blast
{
    ...
    public class NcbiBlastWebHandler : IBlastWebHandler
    {
        ...
        public async Task<Stream> ExecuteAsync(BlastRequestParameters bp, CancellationToken token)
        {
            ...
            {
                seconds = Math.Min(seconds, TimeoutInSeconds);
                Log("Waiting for {0} seconds", seconds);
            ...
            while (true) {
                ...
                {
                    Log("Did not find Status in response: {0}", response);
                }

                // Go to sleep and try again.
                Log("Waiting 2 seconds.", response);
            ...
            Log("Retrieving final response for Request Id {0}", rid);
            return await client.GetStreamAsync(
                string.Format("{0}?CMD=Get&FORMAT_TYPE=XML&RID={1}", EndPoint, rid));
        }
        ...
        public HttpContent BuildRequest(BlastRequestParameters blastParams)
        {
            ...
            if (blastParams.Program == BlastProgram.Megablast)
            {
                ...
                data.Add(CreateKVP("MEGABLAST", "ON"));
            }
            else
                data.Add(CreateKVP("PROGRAM", blastParams.Program));
            data.Add(CreateKVP("DATABASE", blastParams.Database));
